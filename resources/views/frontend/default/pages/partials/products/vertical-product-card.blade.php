...
<div class="vertical-product-card rounded-2 position-relative swiper-slide {{ isset($bgClass) ? $bgClass : '' }}">
    ...
    <div class="thumbnail position-relative text-center p-4">
        ...
        <img src="{{ uploadedAsset($product->thumbnail_image) }}" alt="{{ $product->collectLocalization('name') }}"
            class="img-fluid" id="img-fluid-id">
        </a>
    ...
    <div class="card-content">       
        ...
        <a href="{{ route('products.show', $product->slug) }}"
            id="pname" class="card-title fw-semibold mb-2 tt-line-clamp tt-clamp-1">{{ $product->collectLocalization('name') }}
        </a>

         <!--product category start-->
         <div class="mb-2 tt-category tt-line-clamp tt-clamp-1" id="pcategory">
        ...
        @else
            <form action="" class="direct-add-to-cart-form">
                ...
                @if (!$isVariantProduct && $stock < 1)
                    <button type="button"
                        class="btn btn-outline-secondary btn-md border-secondary d-block mt-4 w-100">{{ localize('Out of Stock') }}</button>
                @else
